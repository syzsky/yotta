...
func main() {
	...
	if *action == "install" {
		expression = `(async()=>{const sleep=ms=>new Promise(r=>setTimeout(r,ms));const button=[...document.querySelectorAll('button')].find(b=>b.textContent?.includes('安装并打开'));if(!button)throw new Error('install action not found');button.click();for(let i=0;i<150;i++){if(location.hash.includes('/edit'))return JSON.stringify({href:location.href});await sleep(100);}throw new Error('installed workflow did not open');})()`
	}
