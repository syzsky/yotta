...
func (s *simulation) HoldForward(context.Context) error {
	if !s.held {
		s.held = true
		s.holds++
	}
...
func (s *simulation) StopForward(context.Context) error {
	if s.held {
		s.held = false
		s.stops++
	}
...
func (s *simulation) Wait(ctx context.Context, d time.Duration) error {
	if s.held {
		return s.Forward(ctx, d)
	}
	return ctx.Err()
}
...
func TestMoveCorrectsDirectionAndConfirmsArrival(t *testing.T) {
	...
	p, d, err := MoveTo(ctx, s, Options{X: 10, Y: 0, Tolerance: 0.6, TurnSign: 1, Pulse: 200 * time.Millisecond, StuckTimeout: time.Second})
...
func TestUnavailableNeverMoves(t *testing.T) {
	...
	_, _, err := MoveTo(context.Background(), s, Options{X: 10, Tolerance: 1, TurnSign: 1, Pulse: 200 * time.Millisecond, StuckTimeout: time.Second})
...
func TestCancellationDuringForwardDoesNotRetry(t *testing.T) {
	...
	_, _, err := MoveTo(ctx, s, Options{X: 10, Tolerance: 1, TurnSign: 1, Pulse: 200 * time.Millisecond, StuckTimeout: time.Second})
...
func TestTurningProgressDoesNotCountAsStuckMovement(t *testing.T) {
	...
	_, distance, err := MoveTo(ctx, s, Options{X: 10, Tolerance: 0.6, TurnSign: 1, Pulse: 200 * time.Millisecond, StuckTimeout: time.Second})
...
func (s *compassSimulation) Wait(ctx context.Context, d time.Duration) error {
	if s.held {
		return s.Forward(ctx, d)
	}
	return ctx.Err()
...
func (s *compassSimulation) Read(ctx context.Context, after time.Time) (Pose, error) {
	p, err := s.simulation.Read(ctx, after)
	p.AxisHeading = 90.7879602977622
	return p, err
}
...
func TestMoveInNTECompassFrame(t *testing.T) {
	...
	_, distance, err := MoveTo(ctx, s, Options{X: -259488, Y: 131920, Tolerance: 20, TurnSign: 1, Pulse: 200 * time.Millisecond, StuckTimeout: time.Second})
